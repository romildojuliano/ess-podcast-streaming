...
export default function ChannelPage() {
  ...
  useEffect(() => {
    const getUserData = async () => {
      const { data } = await (
        await fetch(`http://localhost:4000/getUser/${author}`)
      ).json();
  ...
  const startFollowing = async () => {
    ...
    const { message } = await (
      await fetch(
        `http://localhost:4000/users/${loggedUser}/following?user_to_follow=${author}`,
        {
          method: "POST",
        }
      )
  ...
  const stopFollowing = async () => {
    ...
    const { message } = await (
      await fetch(
        `http://localhost:4000/users/${loggedUser}/following?user_to_unfollow=${author}`,
        {
          method: "DELETE",
        }
      )
    ...
    <Box bgColor="#1E1E1E" h="90vh">
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent bgColor="#252525" color="#eeeeee">
          ...
          <ModalBody>
            <Text>
              Tem certeza que deseja deixar de seguir {userData.author}? você
              não receberá mais notificações de novos podcasts
            </Text>
          </ModalBody>
      ...
      >
        <Flex>
          ...
          >
            <Heading>{userData.author}</Heading>
